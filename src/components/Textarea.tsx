...
interface ContentType {
  ...
  [key: string]: any; // Firestoreのデータ互換性のため
}
...
const Textarea = ({
  ...
  const createContent = async (title: string, text: string, uid: string) => {
  ...
  const onTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setCurrentTargetContent((prev: ContentType) => ({
      ...prev,
  ...
  const onTextChangeMonaco = (value: string | undefined) => {
    setCurrentTargetContent((prev: ContentType) => ({
      ...prev,
      text: value || "",
    }));
  ...
  const onTabKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === "Tab") {
      ...
      if (!(target instanceof HTMLTextAreaElement)) {
        console.error("Target is not a textarea");
        return;
      }
